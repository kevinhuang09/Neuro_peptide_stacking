...
class Stacking:
    ...
    def genSelfTestResult(self, drawPlot=False):
        ...
        # --- 修改開始：直接對全部訓練資料 X 進行預測 ---
        # 原本這裡有 train_test_split，我們直接跳過，用 self.dataTrain_X
        selfTestPredObj = Predict(dataX=self.dataTrain_X, modelList=loadModelList)
        selfTestPredVectorList, self.selfTestProbVectorList = selfTestPredObj.doPredict()
        
        # Scoring 也要對應改成全體的 y
        selfTestScoreObj = Scoring(predVectorList=selfTestPredVectorList, 
                                   probVectorList=self.selfTestProbVectorList,
                                   answerDf=self.dataTrain_y, 
        ...
        if drawPlot:
            # 這裡也要改用全體資料繪圖
            drawSelfTestObj = DrawPlot(answerDf=self.dataTrain_y, modelList=loadModelList, 
                                       modelNameList=self.modelNameList, 
                                       predArrList=selfTestPredVectorList, 
                                       probArrList=self.selfTestProbVectorList)
            drawSelfTestObj.drawROC()
